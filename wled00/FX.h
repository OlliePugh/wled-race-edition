...
private:
  ...
  struct Transition
  {
    ...
    Transition(uint16_t dur = 750)
        : _palT(CRGBPalette16(CRGB::Black)), _prevPaletteBlends(0), _start(millis()), _dur(dur)
    {
    }
  } *_t;
...
public:
  Segment(uint16_t sStart = 0, uint16_t sStop = 30) : start(sStart),
                                                      stop(sStop),
                                                      offset(0),
                                                      speed(DEFAULT_SPEED),
...
// Serial.println();
  ...
  inline void blurCols(fract8 blur_amount, bool smear = false)
  { // blur all columns
  ...
  inline unsigned nrOfVStrips() const
  { // returns number of virtual vertical strips in 2D matrix (used to expand 1D effects into 2D)
...
public:
  ...
  inline void restartRuntime()
  {
    for (Segment &seg : _segments)
    {
      seg.markForReset().resetIfRequired();
    }
  }
  inline void setTransitionMode(bool t)
  {
    for (Segment &seg : _segments)
  ...
  inline void fill(uint32_t c)
  {
    for (unsigned i = 0; i < getLengthTotal(); i++)
      setPixelColor(i, c);
  ...
  inline uint16_t getMappedPixelIndex(uint16_t index) const
...
#define WLED_MAX_PANELS 18
  ...
  typedef struct panel_t
  {
    ...
    panel_t()
        : xOffset(0), yOffset(0), width(8), height(8), options(0)
    {
    }
  } Panel;
